...
class CommunityModelFindGolfers extends JCCModel
    implements CLimitsInterface, CNotificationsInterface
{
	...
	public function getCourseSearch( $serkey )
	{
		...
		if( !is_null($serkey) )
		{
			$extraSQL	= " AND (a.Name Like " . $db->Quote('%'.$serkey.'%') . " OR a.City Like ". $db->Quote('%'.$serkey.'%') . ") ";
		}
    ...
    public function getsdateRealTime($sd)
    {
        ...
        $query = "SELECT sdate from " . $db->quoteName('#__gwm_golfers_schedule_date') . " WHERE id=" . $db->Quote($sd);
        $db->setQuery($query);
        $db->execute();
        $sd = $db->loadResult();
        ...
        public function getTableValue($tableid, $text)
        {
            if (strpos($text, ",") > 0)
            ...
            switch ($tableid) {
                ...
                case "handicap":
                    //echo "x:" . $text . ":x";
                    switch ($text) {
                        case "Championship golfer - Score 60 - 75":
                            return 1;
                            break;
                        case "1st flight golfer - Score 76 - 85":
                            return 2;
                            break;
                        case "2nd flight golfer - Score 86 - 95":
                            return 3;
                            break;
                        case "3rd flight golfer - Score 96 - 105":
                            return 4;
                            break;
                        case "4th flight golfer - Score over 106":
                            return 5;
                            break;
                        default:
                            return 0;
	...
	public function addFindGolfers($id, $fieldset)
        {
            $my = CFactory::getUser();
            $db= $this->getDBO();
            ...
            //$profileField = $data->profile ['fields']['Basic Information'] ;
            //print_r($profileField);
            $gender='';
            $birth='';
            $agegroup='';
            $golfwithsex='';
            ...
            foreach ($data->profile['fields'] as $key => $fields) {	
                foreach ($fields as $key2 => $field) {
                    ...
                    // Requestors Gender
                    if($fieldname=="Gender")
                        $gender=JText::_($field['value']);
                    
                    // Requestors Birthdate
                    if($fieldname=="Birthdate")
                        $birth=JText::_($field['value']);

                    // The age the requestor would like to golf with
                    if($fieldcode=="FIELD_AGEPREFERENCE")
                        $agegroup=JText::_($field['value']);

                    // Requestors playing sex preference
                    if($fieldcode=="FIELD_GENDERFIND")
                        $golfwithsex=JText::_($field['value']);

                    // Average score of the requestor
                    if($fieldcode=='FIELD_SCOREFIND')
                        $handicap=JText::_($field['value']);
                    
                    // Average score the requestor like to golf with
                    if($fieldcode=='FIELD_SCORE')
                        $handicappref=JText::_($field['value']);
                    
                }

            }
            //echo $gender . " x " . $birth . " x " . $agegroup . " x " . $golfwithsex . " x " . $handicap;
            
            $age = date_diff(date_create($birth), date_create('today'))->y;
            
            // where to pick the search cirteria from:
            // if serpar == serpar then the search parameters box was check, use it, if not use the preferences
            if ($fieldset['serpar'] == "serpar") {
                // use the search parameters
                $sexsearch = $fieldset['sex'];
                $agesearch = $fieldset['age'];
                $handicapsearch = $fieldset['score'];
            } else {
                // use preferences
                $sexsearch = $golfwithsex;
                $agesearch = $agegroup;
                $handicapsearch = $handicappref;
            }
            
            $date	= JDate::getInstance(); //get the time without any offset!
            $query	= 'INSERT INTO '. $db->quoteName('#__gwm_golfers_schedule')
                ...
                . ', '. $db->quoteName('memneed').' = '. $db->Quote($fieldset['memneed'])
                . ', '. $db->quoteName('responses').' = '. $db->Quote($fieldset['responses'])
                . ', '. $db->quoteName('notify').' = '. $db->Quote($fieldset['notify'])
                . ', '. $db->quoteName('serpar').' = '. $db->Quote($fieldset['serpar'])
                . ', '. $db->quoteName('sex').' = '. $db->Quote($sexsearch)
                . ', '. $db->quoteName('age').' = '. $db->Quote($agesearch)
                . ', '. $db->quoteName('weather').' = '. $db->Quote($fieldset['weather'])
                . ', '. $db->quoteName('handicap').' = '. $db->Quote($fieldset['handicap'])
                . ', '. $db->quoteName('score').' = '. $db->Quote($handicapsearch)
                . ', '. $db->quoteName('uid').' = '. $db->Quote($id)
                . ', '. $db->quoteName('adate').' = ' . $db->Quote($date->toSql())
                . ', '. $db->quoteName('RequestorSex').' = ' . $db->Quote($this->getTableValue("gender", $gender))
                . ', '. $db->quoteName('RequestorSexPreference').' = ' . $db->Quote($this->getTableValue("gender", $sexsearch))
                . ', '. $db->quoteName('RequestorAge').' = ' . $db->Quote($this->getTableValue("agegroup", $age))
                . ', '. $db->quoteName('RequestorAgePreference').' = ' . $db->Quote($this->getTableValue("age", $agesearch))
                . ', '. $db->quoteName('RequestorHandicap').' = ' . $db->Quote($this->getTableValue("handicap", $handicap))
                . ', '. $db->quoteName('RequestorHandicapPreference').' = ' . $db->Quote($this->getTableValue("handicap", $handicapsearch))
            ...
            try {
                ...
                foreach($fieldset['selmember'] as $rowmem) {
                    $query	= 'INSERT INTO '. $db->quoteName('#__gwm_golfers_schedule_member')
                ...
                */
                ...
                foreach($fieldset['datepicker'] as $rowdate) {
                    if($rowdate!='') {
                        $query	= 'INSERT INTO '. $db->quoteName('#__gwm_golfers_schedule_date')
                        .' SET ' . $db->quoteName('gsid').' = '.$db->Quote($gsid)
	...
	public function getExpireSchedule()
	{
		...
		$query	= 'SELECT a.* FROM '
				. $db->quoteName('#__gwm_golfers_schedule') . ' AS a '
				. ' WHERE 1'
				. $extraSQL;
        ...
        {
			$gsid=$result[$i]->id;
			$scheduleDate = $this->getFindGolfersDate($gsid);
			...
			if($totdate==$exp) {
           		//expiry status = 2
				//echo $gsid .'-';
				$query	= 'UPDATE '. $db->quoteName('#__gwm_golfers_schedule')
        ...
        public function getSchedulePendingList( $userId )
	{
            $db		= $this->getDBO();
            ...
            $query	= 'SELECT a.*, fl.sdate, gc.Name FROM '
                            . $db->quoteName('#__gwm_golfers_schedule_accept') . ' AS a '
                            .' join '. $db->quoteName('#__gwm_golfers_schedule_date').' fl on (a.'. $db->quoteName('sdate').'=fl.'. $db->quoteName('id').')'
                            .' join '. $db->quoteName('#__gwm_golfers_schedule').' sc on (a.'. $db->quoteName('gsid').'=sc.'. $db->quoteName('id').')'
    ...
    public function getGolfersScheduleAcceptListWithCreator( $userId )
    {
        $db		= $this->getDBO();
        ...
        $query	= 'SELECT a.* FROM '
                        . $db->quoteName('#__gwm_golfers_schedule_accept') . ' AS a '
                        .' join '. $db->quoteName('#__gwm_golfers_schedule').' fl on (a.'. $db->quoteName('gsid').'=fl.'. $db->quoteName('id').')'
                        . ' WHERE 1'
                        . $extraSQL
                        . $orderBy;
	...
	public function getPendingSchedule( $userid )
	{
		...
		$query	= 'SELECT a.* FROM '
				. $db->quoteName('#__gwm_golfers_schedule_member') . ' AS a '
				.' join '. $db->quoteName('#__gwm_golfers_schedule').' fl on (a.'. $db->quoteName('gsid').'=fl.'. $db->quoteName('id').')'
				. ' WHERE 1'
				. $extraSQL
				. $orderBy;
	...
	public function getMightbeSchedule( $fields )
	{
		$db		= $this->getDBO();
	...
	public function getFindGolfers( $gsid, $useLimit = false )
	{
		$db		= $this->getDBO();
	...
	public function getFindGolfersList( $userId, $useLimit = false )
	{
		$db		= $this->getDBO();
	...
	public function getFindGolfersListClose( $userId)
	{
		$db		= $this->getDBO();
		...
		$query	= 'SELECT a.* FROM '
				. $db->quoteName('#__gwm_golfers_schedule') . ' AS a '
				.' join '. $db->quoteName('#__gwm_golfers_schedule_accept').' fl on (a.'. $db->quoteName('id').'=fl.'. $db->quoteName('gsid').')'
	...
	public function getFindGolfersDate( $gsid)
	{
		$db		= $this->getDBO();
		...
		$query	= 'SELECT a.* FROM '
				. $db->quoteName('#__gwm_golfers_schedule_date') . ' AS a '
				. ' WHERE 1'
				. $extraSQL
				. $orderBy;
	...
	public function getFindGolfersMember( $gsid, $useLimit = false )
	{
		$db		= $this->getDBO();
	...
	public function getFindGolfersMemberCount( $gsid, $status = false )
	{
		$db		= $this->getDBO();
		...
		$query	= 'SELECT COUNT(*) FROM ' . $db->quoteName('#__gwm_golfers_schedule_member') . ' AS a '
				. ' WHERE 1'
				. $extraSQL;
	...
	public function addFindGolfersAccept($id, $fieldset)
    {
        $my = CFactory::getUser();
        $db= $this->getDBO();
		...
		foreach($fieldset['seldate'] as $rowdate) {
			$query	= 'INSERT INTO '. $db->quoteName('#__gwm_golfers_schedule_accept')
		...
		if($total<=0) {
			$query	= 'INSERT INTO '. $db->quoteName('#__gwm_golfers_schedule_member')
			.' SET ' . $db->quoteName('gsid').' = '.$db->Quote($fieldset['reqid'])
			. ', '. $db->quoteName('memid').' = '.$db->Quote($id)
			. ', '. $db->quoteName('status').' = '. $db->Quote(1);
			...
			$query	= 'UPDATE '. $db->quoteName('#__gwm_golfers_schedule_member')
	...
	public function addFindGolfersCancel($id, $fieldset)
    {
        $my = CFactory::getUser();
        $db= $this->getDBO();
	...
	public function addFindGolfersCancelRejected($id, $fieldset)
    {
        $my = CFactory::getUser();
        $db= $this->getDBO();
	...
	public function userScheduleAcceptUpdate($fieldset)
    {
		...
		//delete member date
		$query	= 'DELETE FROM '. $db->quoteName('#__gwm_golfers_schedule_accept')
		...
		*/
		$query	= 'DELETE FROM '. $db->quoteName('#__gwm_golfers_schedule_member')
			.' WHERE '.$db->quoteName('gsid').'='.$db->Quote($fieldset['gsid'])
			.' AND '.$db->quoteName('memid').'='.$db->Quote($my->id);
        $db->setQuery($query);
		$db->execute();
	...
	public function getFindGolfersAcceptCount( $gsid, $sdate, $status = false )
	{
		$db		= $this->getDBO();
		...
		$query	= 'SELECT COUNT(*) FROM ' . $db->quoteName('#__gwm_golfers_schedule_accept') . ' AS a '
				. ' WHERE 1'
				. $extraSQL;
    ...
    public function updateRequestorAccept($id, $fieldset)
    {
		...
		$query	= 'UPDATE '. $db->quoteName('#__gwm_golfers_schedule')
            .' SET ' . $db->quoteName('teetime').' = ' . $db->Quote(date("m-d-Y",strtotime($_REQUEST['chosentime'])) . " " . $fieldset['teetime'])   //$db->Quote($fieldset['teetime'])
		...
		//cancel schedule
		$query	= 'UPDATE '. $db->quoteName('#__gwm_golfers_schedule_accept')
            .' SET ' . $db->quoteName('status').' = '.$db->Quote(0)
		...
		$db->execute();
                ...
                //TODO: Send email to each person accepted to golf
	...
	public function updateRequestDate($fieldset)
    {
		...
		//update group schedule
		$query	= 'UPDATE '. $db->quoteName('#__gwm_golfers_schedule_date')
	...
	public function deleteRequestDate($fieldset)
    {
		...
		//update group schedule
		$query	= 'DELETE FROM '. $db->quoteName('#__gwm_golfers_schedule_date')
			.' WHERE '.$db->quoteName('id').'='.$db->Quote($fieldset['did']);
	...
	public function getFindGolfersAcceptList( $userId )
	{
		$db		= $this->getDBO();
	...
	public function getFindGolfersAcceptListPast( $userId )
	{
		$db		= $this->getDBO();
    ...
    public function getScheduleRequestorAcceptList( $userId )
    {
        ...
        for ($i = 0; $i < count($result); $i++)
        {
            $sDate = $this->getFindGolfersDate($result[$i]->id);
            $actcount=0;
            foreach ( $sDate as $rowd ) {
                    $accmem= $this->getFindGolfersAcceptCount($result[$i]->id,$rowd->id,2);
                    if($accmem>0)
                            $actcount=1;
            }
            if($actcount==1)
    ...
    {
        $smem=$this->getFindGolfersMemberCount($gsid);

        $cancelmem=$this->getFindGolfersMemberCount($gsid,2);
        $acctotmem=$this->getFindGolfersAcceptCount($gsid,$sdate);
    ...
    {
        $smem= $this->getFindGolfersMemberCount($gsid);
        $accmem= $this->getFindGolfersAcceptCount($gsid,$sdate,1);
        $acctotmem= $this->getFindGolfersAcceptCount($gsid,$sdate);
    ...
    {
        $smem= $this->getFindGolfersMemberCount($gsid);
        $accmem= $this->getFindGolfersAcceptCount($gsid,$sdate,2);
        $acctotmem= $this->getFindGolfersAcceptCount($gsid,$sdate);
    ...
    public function getFindGolfersAcceptMember( $gsid, $sdate, $status = false )
    {
        $db		= $this->getDBO();
    ...
    public function getFindGolfersApproveUser( $gsid, $sdate, $status = false )
    {
        $db		= $this->getDBO();
    ...
    {
        $gsData = $this->getFindGolfers($gsid);
    ...
    public function getFindGolfersListMight( $userId )
    {
        //TODO
        $db = $this->getDBO();
        ...
        foreach ($data->profile['fields'] as $key => $fields) {	
            foreach ($fields as $key2 => $field) {
                ...
                if($fieldcode=='FIELD_SEARCH')
                    $golfwithme=JText::_($field['value']);

                if($fieldcode=='FIELDRADIUSFIND')
                    $homegolf=JText::_($field['value']);

                if($fieldcode=='FIELD_WEATHERTYPEFIND')
                    $weather=JText::_($field['value']);
        ...
        $blacklistSQL=' AND a.uid NOT IN(0';
        for ($i = 0; $i < count($result); $i++)
        {
            $blacklistSQL.=', ' . $db->Quote($result[$i]->BlacklistID);
        }
        $blacklistSQL.=') ';

        //blacklist end

        $query	= 'SELECT * FROM ' . $db->quoteName('#__gwm_golfers_schedule_member') . ' AS a '
                        .' WHERE 1 '
                        .' AND '.$db->quoteName('memid').'='.$db->Quote($userId);

        $db->setQuery( $query );
        $result = $db->loadObjectList();

        $extraSQL1=' AND a.id NOT IN(0';
        for ($i = 0; $i < count($result); $i++)
        {
            $extraSQL1.=', ' . $db->Quote($result[$i]->gsid);
        }
        $extraSQL1.=') ';
        ...
        $orderBy	= ' ORDER BY a.id DESC LIMIT 10';	
    /*
        $query	= 'SELECT a.* FROM '
    ...
    */			
        ...
        //age group
        // TODO: make sure that age is looked in both directions
        $age = date_diff(date_create($birth), date_create('today'))->y;
        if($agegroup!='') {
            $expage=explode(",",$agegroup);
             if($expage[0]!="No Preference") {
                $agecon='';
                foreach($expage as $val) {
                    if($val!='') {
                        $agegroupnum = $this->getTableValue("age", $agegroup);
                        //echo "agegroupnum: " . $agegroupnum . "  agegroup: " . $agegroup;
                        $agerange = $this->getTableValue("agegroupvalues", $agegroupnum);
                        //echo "agerange: " . $agerange;
                        $ageranges = explode("-", $agerange);
                        
                        if($agecon=="") {
                            //$agecon=' a.age Like '.$db->Quote("%".$val."%");
                            $agecon=' (a.RequestorAge >= ' . $db->Quote($ageranges[0]) . ' and a.RequestorAge <= ' . $db->Quote($ageranges[1]) . ')';
                        } else { 
                            $agecon=' or (a.RequestorAge >= ' . $db->Quote($ageranges[0]) . ' and a.RequestorAge <= ' . $db->Quote($ageranges[1]) . ')';
                        }
                    }
                }
                $extraCon.=" AND ( ".$agecon." )";
        ...
        // age group preference of requestor
        $age = date_diff(date_create($birth), date_create('today'))->y;
        $myagegroup = $this->getTableValue("agegroup", $age);
        $extraCon.=" AND ( a.RequestorAgePreference = 0 or a.RequestorAgePreference = " . $db->Quote($myagegroup) . " )";

        //gender
        // TODO: make sure gender preference is looked at in both directions
        if($golfwithsex!='') {
            if($golfwithsex!="No Preference" and $golfwithsex!="No preference") {
                    $extraCon.=" AND a.RequestorSex=". $db->Quote($this->getTableValue("gender", $golfwithsex));
            }
        }

        // consider the requestor sex preference
        
        $extraCon.=" AND ( a.RequestorSexPreference = 0 or a.RequestorSexPreference = " . $db->Quote($this->getTableValue("gender", $gender)) . " )";
        
        
        //score
        // TODO: make sure score is looked at in both directions
        if($score!='') {
            $expage=explode(",",$score);
            if($expage[0]!="No Preference") {
                $agecon='';
                foreach($expage as $val) {
                    if($val!='') {
                        $handicapgroupnum = $this->getTableValue("handicap", $score);
                        if($agecon=="")
                            $agecon=' a.RequestorHandicap = '.$db->Quote($handicapgroupnum);
                        else 
                            $agecon.=' OR a.RequestorHandicap = '.$db->Quote($handicapgroupnum);
                    }
                }
                if($agecon!='')
                    $extraCon.=" AND ( ".$agecon." )";
        ...
        $extraCon.=" AND ( a.RequestorHandicapPreference = 0 or a.RequestorHandicapPreference = " . $db->Quote($this->getTableValue("handicap", $handicap)) . " )";
        
        //weather
        //echo $weather;
        //if($weather!='') { 
        //    if(strpos($weather, "No Preference") != false) {
        //        $extraCon.=" AND a.weather=". $db->Quote($weather);
        //    }
        //}


        if (strpos($homegolf, "miles") != false) {
            
            $miles = trim(substr($homegolf,(strpos($homegolf, "miles")-4),3));
            //echo $miles;
            //get user lat/lon
            $latlng = $model->getUserLatLon($userId);
            //print_r($latlng);
            $latlonSQL = ", (3959 * acos(cos(radians('" . $latlng->Lat . "')) * cos(radians(fl.Lat)) * cos(radians(fl.Lon) - radians('" 
                    . $latlng->Lon . "')) + sin(radians('" . $latlng->Lat . "')) * sin(radians(fl.Lat)))) as distance";

            $distanceSQL = " AND distance < " . $miles;
        } else {
            $latlonSQL = "";
            $distanceSQL = "";
        }
        
        $query = 'SELECT a.*,fl.'. $db->quoteName('Name') . ',fl.' . $db->quoteName('Lat') . ',fl.' . $db->quoteName('Lon') . $latlonSQL
            .' FROM	'. $db->quoteName('#__gwm_golfers_schedule').' a'
            ...
            .' HAVING a.'. $db->quoteName('status').'='.$db->Quote(1)
            .' AND a.'. $db->quoteName('uid').'!='.$db->Quote($userId)
            . $extraSQL1
            . $extraCon
            . $blacklistSQL
            . $distanceSQL
